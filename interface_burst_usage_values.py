...
columns = ['SITE NAME', 'ROUTER', 'INTERFACE', 'IN 0 - 30%', 'IN 30 - 60%' , 'IN 60 - 80%', 'IN 80 - 100%']
...
for interface in interfaces['records']:
   ...
   for app in response['records']:
      if (row_num<65536):
         ...
         ws.write(row_num, 3, app['InBurst1'] , center_style)
         ws.write(row_num, 4, app['InBurst2'] , center_style)
         ws.write(row_num, 5, app['InBurst3'] , center_style)
         ws.write(row_num, 6, app['InBurst4'] , center_style)
         row_num = row_num + 1

wb.save('interface_burste_usage_new' + str(local_timestamp) + '.xls')
