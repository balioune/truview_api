...
columns = ['SITE NAME', 'ROUTER', 'INTERFACE', 'INT DESC', 'APPLICATION', 'APP DESC' , '% BW Usage', 'TotalThroughput']
...
for interface in interfaces['records']:
   ...
   if "chart" in response.keys():
      ...
      for app in response['records']:
         #print(interface['Site'][0]['name'])
         #print(interface['Device']['name'], interface['Interface']['name'] )
         #print(app)
         #time.sleep(3)
         ...
         ws.write(row_num, 6, app['TotalUtilization'] , center_style)
         ws.write(row_num, 7, app['TotalThroughput'] , center_style)
...
with open('app_interfaces_usage' + str(local_timestamp) + '.json', 'w') as f:
    json.dump(jsondata, f, indent=2)

wb.save('app_interfaces_usage' + str(local_timestamp) + '.xls')
